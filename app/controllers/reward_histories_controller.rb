class RewardHistoriesController < ApplicationController
	def index_p5
		...
		@user = User.find(params[:id])
		@p5_histories = @user.given_rewards.includes(:receiver)
	end

	def index_rewards
		@user = User.find(params[:id])
		@reward_histories = @user.received_rewards.includes(:giver)
	end
	...
	def create
		...
		if points <= giver.p5_balance && points <= 100
			RewardHistory.create!(datetime: Time.current, points: points, given_by: giver.id, given_to: receiver.id)
			giver.update!(p5_balance: giver.p5_balance - points)
			receiver.update!(reward_balance: receiver.reward_balance + points)
			redirect_to root_path(giver), notice: 'Reward created successfully!'
		else
			redirect_back fallback_location: root_path, alert: 'Insufficient balance or invalid points!'
		end
	end
